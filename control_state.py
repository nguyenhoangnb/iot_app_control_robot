...
class main_control(QMainWindow):
    def __init__(self):
        ...
        self.conn = sqlite3.connect("states_manual.db")
        ...
        self.current_state = ""
        self.state_topic_maunal = "state_data"
        self.main_handle.lable_app.setText("Điều khiển")
        self.broker_address = "192.168.43.111"
        self.port = 1883  # Default MQTT port
        try:
            self.mqtt_client.connect(self.broker_address, self.port)
            self.mqtt_client.subscribe("handleTopicManual")
        ...
        self.main_handle.control_v.clicked.connect(self.show_control_dialog)  # Kết nối nút control_v
    def on_message(self, client, userdata, msg):
        ...
        if msg.topic == "handleTopicManual":
    ...
    def show_control_dialog(self):
        """
        Hiển thị giao diện điều khiển vận tốc.
        """
        self.control_dialog = ControlDialog(self.mqtt_client, self)
        self.control_dialog.exec_()
    ...
    def publish_command(self, state_data):
        try:
            ...
            self.mqtt_client.publish(self.state_topic_maunal, message)
            print(f"Published: {message} to topic: {self.state_topic_maunal}")
        except Exception as e:
            print(f"Error publishing to topic {self.state_topic_maunal}: {e}")
